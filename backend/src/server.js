...
// 3. 정보 조회 API (이동할 경로 확인)
app.get('/api/users/:userId/route', async (req, res) => {
    ...
    let targetPage = '/';
    if (user.role_type === 'SME') targetPage = '/sme-dashboard';
    else if (user.role_type === 'FUNDER') targetPage = '/funder-market';
...
app.get('/api/mypage/:userId', async (req, res) => {
    ...
    // [수정 후] 지갑 데이터 가져올 때 잔액도 같이 가져오기
    const { data: wallet } = await supabase
        .from('wallets')
        .select('wallet_address, rlusd_balance') // 잔액(rlusd_balance) 컬럼 추가
        .eq('owner_id', userId)
        .single();
...
app.listen(process.env.PORT, () => {
    console.log(`백엔드 서버 구동 완료, 포트: ${process.env.PORT}`);
});